...
class BaseCommon(unittest.TestCase):
    ...
    @classmethod
    def request(self, method: str, url, params=None, data=None, json=None, timeout=5, **kwargs):  # 自定义发送请求
        ...
        if method == "GET":  # 当请求方法为GET时，调用requests库中的get请求
            ...
            if json:
                ...
                log_common.log(f"请求方法：{method}，请求地址：{url}，请求参数：{json}，响应结果：{res.text}")
            elif params:
                # logger.info(f"请求方法：{method}，请求地址：{url}，请求参数：{params}，响应结果：{res.text}")
                log_common.log(f"请求方法：{method}，请求地址：{url}，请求参数：{params}，响应结果：{res.text}")
            ...
            res = requests.post(url, data=data, json=json, timeout=timeout, **kwargs)
            if data:
                # logger.info(f"请求方法：{method}，请求地址：{url}，请求参数：{data}，响应结果：{res.text}")
                log_common.log(f"请求方法：{method}，请求地址：{url}，请求参数：{data}，响应结果：{res.text}")
            elif json:
                # logger.info(f"请求方法：{method}，请求地址：{url}，请求参数：{json}，响应结果：{res.text}")
                log_common.log(f"请求方法：{method}，请求地址：{url}，请求参数：{json}，响应结果：{res.text}")
            return res
        elif method == "PUT":  # 当请求方法为PUT时，调用requests库中的PUT请求
            res = requests.put(url, data=data, json=json, timeout=timeout, **kwargs)
            if data:
                # logger.info(f"请求方法：{method}，请求地址：{url}，请求参数：{data}，响应结果：{res.text}")
                log_common.log(f"请求方法：{method}，请求地址：{url}，请求参数：{data}，响应结果：{res.text}")
            elif json:
            ...
            res = requests.delete(url, data=data, json=json, timeout=timeout, **kwargs)
            if data:
                # logger.info(f"请求方法：{method}，请求地址：{url}，请求参数：{data}，响应结果：{res.text}")
                log_common.log(f"请求方法：{method}，请求地址：{url}，请求参数：{data}，响应结果：{res.text}")
            elif json:
